...
char* getResolvedValues(char *hostname, char *nameserver){
	...
	if(ipaddress_received != NULL){
		...
		return getResolvedValues(nslookup_received,rootserver);
	}
	exit(1);
} 

int check_if_dns_working(char *dnsserver,char* hostname){
	// using a constant name server address for now.
	in_addr_t nameserver_addr=inet_addr(dnsserver);
...
char* parseInputData(int argc,char* argv[]){
	...
	int root_server_working =0;
	static const char filename[] = "root-servers.txt";
   	FILE *file = fopen ( filename, "r" );
   	if ( file != NULL )
   	{
      char line [ 128 ]; /* or other suitable maximum line size */
      while ( fgets ( line, sizeof line, file ) != NULL && root_server_working == 0) /* read a line */
      {
      	if(check_if_dns_working(line,TEST_HOST) == 1){
      		root_server_working = 1;
			rootserver=(char*)malloc(sizeof(line));
			strcpy(rootserver,line);
      	}      
      }
      fclose ( file );
   	}
    ...
    getResolvedValues(hostname,rootserver);
}
