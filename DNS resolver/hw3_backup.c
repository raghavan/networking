...
char* getResolvedValues(int argc, char* argv[]){

	/*while(*argv != NULL){
		printf("argv in passed place --> %s\n",*argv++);
	}*/
	...
	for(a=0;a<answer_count+auth_count+other_count;a++) {
		...
		if(htons(rr->type)==RECTYPE_A) {
			template = (char *)malloc(strlen("The name ")+strlen(string_name)+strlen(" resolves to IP addr: ")+strlen(inet_ntoa(*((struct in_addr *)answer_ptr))));
			printf("The name %s resolves to IP addr: %s\n",
						 string_name,
						 inet_ntoa(*((struct in_addr *)answer_ptr)));
			sprintf(template,tpl,"The name ", string_name," resolves to IP addr: ",inet_ntoa(*((struct in_addr *)answer_ptr)));
			got_answer=1;
			return template;
		}
		// NS record
...
while(argv[i] != NULL){
	if(strcmp(argv[i],"-d")==0){
		...
		j++;
		newargv[j] = malloc(sizeof("128.248.7.50"));
		memcpy(newargv[j], "128.248.7.50", sizeof("128.248.7.50"));
		j++;
	}else{
		newargv[j] = malloc(sizeof(argv[i]));
...
newargv[j] = NULL;
/*int k=0;
for(k;k<sizeof(newargv);k++){
	printf("new argv -->%s\n",newargv[k]);
}*/
argc += 2;
char* resolvedIp = getResolvedValues(argc,newargv);
printf("Resolved IP --> %s\n",resolvedIp );

}else{
	//printf("argc --> %d",argc);
	char* resolvedIp = getResolvedValues(argc,argv);
	printf("Resolved IP --> %s\n",resolvedIp );
}
